...
function QUERY_CLASS:Escape(text)
	return Athena.mysql:Escape(tostring(text));
end;
...
function QUERY_CLASS:Execute(bQueueQuery)
	...
	if (type(queryString) == "string") then
		if (!bQueueQuery) then
			return Athena.mysql:RawQuery(queryString, self.callback);
		else
			return Athena.mysql:Queue(queryString, self.callback);
		end;
	end;
end;
...
function Athena.mysql:Select(tableName)
	return QUERY_CLASS:New(tableName, "SELECT");
end;

function Athena.mysql:Insert(tableName)
	return QUERY_CLASS:New(tableName, "INSERT");
end;

function Athena.mysql:Update(tableName)
	return QUERY_CLASS:New(tableName, "UPDATE");
end;

function Athena.mysql:Delete(tableName)
	return QUERY_CLASS:New(tableName, "DELETE");
end;

function Athena.mysql:Drop(tableName)
	return QUERY_CLASS:New(tableName, "DROP");
end;

function Athena.mysql:Truncate(tableName)
	return QUERY_CLASS:New(tableName, "TRUNCATE");
end;

function Athena.mysql:Create(tableName)
	...
	elseif (Module == "mysqloo") then
		...
		if (mysqloo) then
			...
			self.connection.onConnected = function(database)
				Athena.mysql:OnConnected();
			end;

			self.connection.onConnectionFailed = function(database, errorText)
				Athena.mysql:OnConnectionFailed(errorText);
			end;		
...
function Athena.mysql:RawQuery(query, callback, flags, ...)
...
function Athena.mysql:Queue(queryString, callback)
	if (type(queryString) == "string") then
...
function Athena.mysql:SetModule(moduleName)
	Module = moduleName;
end;

-- Called when the database connects sucessfully.
function Athena.mysql:OnConnected()
	MsgC(Color(25, 235, 25), "[Athena] Connected to the database!\n");

	Connected = true;
	hook.Run("Athena_DatabaseConnected");
end;

-- Called when the database connection fails.
function Athena.mysql:OnConnectionFailed(errorText)
	ErrorNoHalt("[Athena] Unable to connect to the database!\n"..errorText.."\n");

	hook.Run("Athena_DatabaseConnectionFailed", errorText);
end;

-- A function to check whether or not the module is connected to a database.
function Athena.mysql:IsConnected()
	return Connected;
end;

timer.Create("Athena.mysql.Think", 1, 0, function()
	Athena.mysql:Think();
end);